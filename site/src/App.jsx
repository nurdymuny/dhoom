...
const DOCS = {
  "getting-started": {
    ...
    content: [
      ...
      "### What\u2019s New in v0.5",
      "v0.5 introduces three features from geometric fiber bundle theory:",
      "- **String interning (`&`)** \u2014 Pool repeated strings into an associated bundle. Records use integer indices.\n- **Computed fields (`#`)** \u2014 Fields derived from other fields via expressions. Omitted from records. Sheaf sections.\n- **Inline constraints (`!`)** \u2014 Type/validation annotations (`!int`, `!str`, `!enum:a/b`). Metadata only. Section conditions.",
      "### Previous: v0.4",
      "v0.4 introduced three features from geometric fiber bundle theory:",
  ...
  "math": {
    ...
    content: [
      ...
      "### v0.5: Three More Geometric Operations",
      "**8. Associated bundles (& interning):** The string pool defines a discrete fiber, and record values are sections of the **associated bundle** \u2014 integer indices into the pool replace repeated string values.",
      "**9. Sheaf sections (# computed):** Computed fields are **global sections** determined by transition rules over other fibers. The expression `total#price*qty` defines a sheaf morphism: the section is fully determined by the local data.",
      "**10. Section conditions (! constraints):** Type annotations restrict which sections are **admissible** \u2014 constraints on the fiber that don\u2019t change encoding but document the structure.",
  ...
  "string-interning": {
    title: "String Interning (&)", cat: "Guide",
    content: [
      "## String Interning",
      "The `&` modifier declares a field as **interned**. Repeated string values are pooled into a vocabulary line, and records use integer indices instead of the full strings.",
      "### Why Intern?",
      "When a string field has many repeated values (e.g., status codes, categories, country names), transmitting the full string every time wastes space:",
      C + "dhoom\n# Without interning: repeated strings\norders{id@1, status}:\ncompleted\npending\ncompleted\ncompleted\nfailed\ncompleted\npending\ncompleted\ncompleted\n" + C,
      C + "dhoom\n# With interning: pool + indices\norders{id@1, status&}:\n&status[completed, pending, failed]\n0\n1\n0\n0\n2\n0\n1\n0\n0\n" + C,
      "### Pool Line Syntax",
      "The pool line appears immediately after the header, before any records:",
      "`&fieldname[value0, value1, value2, ...]`",
      "Record values are **zero-based indices** into this pool. The decoder resolves each index back to the corresponding string.",
      "### The Geometry",
      "Interning models an **associated bundle**. The pool defines a discrete fiber (a finite set of strings), and each record\u2019s index is a section of that associated bundle. The pool line declares the fiber; records encode sections as elements of the index set.",
      "### Encoder Detection",
      "The encoder automatically detects internable fields when:\n- All values are strings\n- There are \u22653 records\n- The number of distinct values \u2264 \u2308N/3\u2309\n- There are \u22652 distinct values\n- Interning saves \u226510% of characters vs raw strings",
      "### When to Use &",
      "Use `&` when:\n- Status codes, categories, or enum-like values repeat frequently\n- Country names, department names, or other vocabularies\n- Any string field where the distinct count is much smaller than the record count",
      "### When NOT to Use &",
      "Do not use `&` when:\n- Values are mostly unique (names, emails, UUIDs)\n- The pool would be almost as large as the data\n- There are fewer than 3 records",
    ].join("\n\n"),
  },

  "computed-fields": {
    title: "Computed Fields (#)", cat: "Guide",
    content: [
      "## Computed Fields",
      "The `#` modifier declares a field whose value is **derived from other fields** via an expression. Computed fields are **omitted from records** \u2014 the decoder evaluates the expression to reconstruct them.",
      "### Why Computed?",
      "When a field is always the product, sum, or difference of other fields, transmitting it is pure redundancy:",
      C + "dhoom\n# Without computed: total is redundant\nitems{price, qty, total}:\n10, 3, 30\n20, 5, 100\n15, 2, 30\n" + C,
      C + "dhoom\n# With computed: total derived automatically\nitems{price, qty, total#price*qty}:\n10, 3\n20, 5\n15, 2\n" + C,
      "`total#price*qty` means total = price \u00d7 qty. The field vanishes from records.",
      "### Supported Operators",
      "| Operator | Meaning | Example |\n|---|---|---|\n| `*` | Multiply | `total#price*qty` |\n| `+` | Add | `sum#a+b` |\n| `-` | Subtract | `diff#gross-tax` |",
      "Expressions are always `field1 <op> field2` \u2014 two field names and one binary operator.",
      "### The Geometry",
      "Computed fields model **sheaf sections**. The expression defines a sheaf morphism: the computed field\u2019s value is a **global section** fully determined by the transition rule over the other fibers. It carries zero independent information.",
      "### Encoder Detection",
      "The encoder automatically detects computed fields by:\n1. Identifying all remaining numeric fields after arithmetic categorization\n2. Testing every pair of fields with every operator (`+`, `-`, `*`)\n3. Checking if the expression matches all record values exactly\n4. Emitting `#expr` when a match is found",
      "### When to Use #",
      "Use `#` when:\n- A field is always the product/sum/difference of two other fields\n- The relationship holds across **every** record\n- The values are numeric",
      "### When NOT to Use #",
      "Do not use `#` when:\n- The expression doesn\u2019t hold for every record\n- The relationship involves more than two fields\n- Values are non-numeric",
    ].join("\n\n"),
  },

  "inline-constraints": {
    title: "Inline Constraints (!)", cat: "Guide",
    content: [
      "## Inline Constraints",
      "The `!` modifier attaches type or validation metadata to a field. Constraints are **purely declarative** \u2014 they don\u2019t change encoding or decoding.",
      "### Syntax",
      "`field!type` \u2014 where type is one of:",
      "| Constraint | Meaning |\n|---|---|\n| `!int` | Integer values |\n| `!num` | Numeric values (int or float) |\n| `!bool` | Boolean values |\n| `!str` | String values |\n| `!enum:val1/val2/...` | One of the listed values (uses `/` separator) |",
      "### Example",
      C + "dhoom\nusers{name!str, age!int, role!enum:admin/editor/viewer}:\nAlice, 30, admin\nBob, 25, viewer\nCarol, 28, editor\n" + C,
      "The constraints declare that `name` is a string, `age` is an integer, and `role` must be one of admin/editor/viewer.",
      "### The Geometry",
      "Constraints model **section conditions** \u2014 restrictions on which sections are admissible over the fiber. They constrain the space of valid data without affecting the encoding of specific sections.",
      "### Semantics",
      "- Constraints are **metadata only** \u2014 a decoder treats constrained fields identically to unconstrained ones\n- The `!` modifier is available for schema validation, documentation, and tooling\n- The encoder does **not** auto-detect constraints \u2014 they must be declared explicitly\n- A field may have at most one modifier; `!` is mutually exclusive with `@`, `|`, `>`, `^`, `->`, `&`, `#`",
      "### Enum Separator",
      "Enum values use `/` as the separator (not `,` which conflicts with fiber header commas):\n\n`role!enum:admin/editor/viewer` \u2014 three valid values",
    ].join("\n\n"),
  },
  ...
  "implementations": {
    ...
    content: [
      ...
      "The encoder detects arithmetic sequences, computes modal defaults, detects delta-encodable fields, identifies sparse bundles, detects interned and computed fields, orders fields for trailing elision, and emits records with deviation marking.",
      "### Contributing",
...
const SIDEBAR = [
  { heading: "Guide", items: [
    ...
    { id: "string-interning", label: "String Interning (&)" },
    { id: "computed-fields", label: "Computed Fields (#)" },
    { id: "inline-constraints", label: "Inline Constraints (!)" },
    ...
    <nav style={{ position: "sticky", top: 0, zIndex: 100, background: "rgba(8,6,14,0.94)", backdropFilter: "blur(12px)", borderBottom: "1px solid rgba(255,255,255,0.04)", padding: "0 16px" }}>
      <div style={{ height: 52, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          {!mob && <span style={{ fontSize: 11, color: "#383850", fontFamily: "monospace" }}>v0.5</span>}
...
function FeatureCards() {
  const feats = [
    ...
    { i: "&", t: "String Interning", d: "Pool repeated strings. Records use integer indices." },
    { i: "#", t: "Computed Fields", d: "Derived from other fields. Omitted from records." },
    { i: "!", t: "Inline Constraints", d: "Type annotations. Metadata only." },
      ...
      <div style={{ display: "grid", gridTemplateColumns: mob ? "1fr 1fr" : "repeat(3,1fr)", gap: 10 }}>
        {feats.map((f, i) => (
          <div key={i} style={{ background: "rgba(255,255,255,0.015)", border: "1px solid rgba(255,255,255,0.035)", borderRadius: 10, padding: "18px 16px" }}>
            ...
            <div style={{ fontSize: 14, fontWeight: 700, color: "#B0B0C8", margin: "8px 0 4px" }}>{f.t}{i >= 5 && i <= 10 && <span style={{ fontSize: 9, marginLeft: 6, padding: "1px 5px", borderRadius: 3, background: "#FF603018", color: "#FF6030", fontWeight: 700, fontFamily: "'Fira Code',monospace", verticalAlign: "middle" }}>{i >= 8 && i <= 10 ? "v0.5" : "v0.4"}</span>}</div>
